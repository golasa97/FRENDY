...
void ResonanceXSCalculator::check_ene_data(int i, int j, vector<Real8>& ene_data)
{
  ...
  if( i==0 && j==0 )
  {
    el = reso_data_obj.get_lower_ene_limit()[i][j] * (1.0 - min_ene_dif);
  }
  ...
  for(int k=0; k<k_max; k++)
  {
    ene_data[k] = digit_obj.get_truncation_value(ene_data[k]);
    ...
    else if( warn_flg == 0 )
    {
      ...
      string str_data05 = "Set energy grid data                   : " + oss07.str();
...
void ResonanceXSCalculator::calc_reso_xs_each_case(int i, int j, Real8& ene_val, vector<Real8>& sig_val)
{
  ...
  {
    sig_val[l] = digit_obj.get_truncation_value(sig_val[l]);
    if( sig_val[l] < 0.0 )
    {
      sig_val[l] = 0.0;
    }
...
void ResonanceXSCalculator::calc_reso_xs_bw_single(int i, int j, Real8& ene_val, vector<Real8>& sig_val)
{
  ...
  for(int l=0; l<l_max; l++)
  {
    ...
    if( lrx_multi[l] != 0 )
    {
      ...
      calc_shift_penetration_factor(l_mod, rho_qx, sl_qx, pl_qx);
      if( e_qx < 0.0 )
      {
        pl_qx = 0.0;
      }
    ...
    for(int m=0; m<m_max; m++)
    {
      ...
      if( temp > 0.0 )
      {
        ...
        if( lrx_multi[l] != 0 )
        {
          sig_lrx             +=  smr * gx_er_multi[l][m]*pl_qx * psi;
        }
      ...
      else
      {
        ...
        if( lrx_multi[l] != 0 )
        {
          sig_lrx             +=  comfac * gx_er_multi[l][m]*pl_qx;
        }
      }
    }
  ...
  if( fabs(sig_lrx) > min_value )
  {
    sig_val[total_xs] += sig_lrx;
  }
}
...
void ResonanceXSCalculator::calc_reso_xs_bw_multi(int i, int j, Real8& ene_val, vector<Real8>& sig_val)
{
  ...
  for(int l=0; l<l_max; l++)
  {
    ...
    for(int m=0; m<m_max; m++)
    {
      ...
      if( aj_no == static_cast<int>(sigj.size()) )
      {
        aj_no--;
      }
      else if( aj_no == -1 )
...
void ResonanceXSCalculator::set_multi_array_data_bw(int i, int j)
{
  ...
  k_max = static_cast<int>(qx_multi.size());
  for(int k=0; k<k_max; k++)
  {
//    qx_multi[k] = qx_multi[k] * (awri[i][j][k] + 1.0) / awri[i][j][k];
  }
}
                                                           ...
                                                           Real8& mid_ene, vector<Real8>& mid_sig )
{
  ...
  for(int l=1; l<l_max; l++)
  {
    ...
    Real sig_half = 0.0;
    ti_obj.interpolation_1d(int_lin_lin, mid_ene, sig_half,
                            ene_data[ele_no-1], sig_data[ele_no-1][l],
                            ene_data[ele_no],   sig_data[ele_no][l]);
    sig_half = digit_obj.get_truncation_value(sig_half);
    delta_sig[l] = fabs(mid_sig[l] - sig_half);
    sig_coef[l]  = fabs(sig_data[ele_no][l]);
    if( sig_coef[l] > fabs(sig_data[ele_no-1][l]) )
    {
      sig_coef[l] = fabs(sig_data[ele_no-1][l]);
    }
    if( sig_coef[l] > fabs(mid_sig[l]) )
    {
      sig_coef[l] = fabs(mid_sig[l]);
    }

    if( delta_sig[l] < err * sig_coef[l] )
    {
      chk_flg++;
    }
  ...
  else
  {
    ...
    for(int l=1; l<l_max; l++)
    {
      if( delta_sig[l] > err * sig_coef[l] )
      {
        if( delta_sig[l] > err_max * sig_coef[l] )
        {
          chk_flg = -1;
          break;
        }
    ...
    if( ene_data[ele_no]/ene_data[ele_no-1] > 1.0+sqrt(5.3*error_value) )
    {
      return -100;
    }
  }
...
void ResonanceXSCalculator::calc_const_value_bw(int i, int j)
{
  ...
  for(int k=0; k<k_max; k++)
  {
    ...
    for(int l=0; l<l_max; l++)
    {
      ...
      if( lrx_val != 0 )
      {
        Real8 wave_no_qx, e_qx, rho_qx, rho_h_qx, sl_qx, pl_qx;
        e_qx = er + bw_data_obj.get_q_value()[k];
        wave_no_qx = wave_no_part[i][j][k] * sqrt(fabs(e_qx));
        calc_rho(i, j, k, e_qx, wave_no_qx, rho_qx, rho_h_qx);
        ...
        gx_er[i][j][k][l] = (gt - (gn + gg + gf)) / pl_qx;
        if( (gt - (gn + gg + gf)) < 1.0E-5 * gt )
        {
          gx_er[i][j][k][l] = 0.0;
        }
      }
